export const translations = {
  da: {
    analytics: {
      title: 'Hjælp os med at forbedre siden',
      description: 'Vi bruger anonymiserede brugsdata til at forstå, hvad der fungerer på siden. Statistik aktiveres kun, hvis du accepterer.',
      accept: 'Accepter statistik',
      decline: 'Afvis'
    },
    seo: {
      title: 'Ballen Fisk | Frisk fisk og røgvarer på Samsø',
      description: 'Ballen Fisk ved smukke Ballen Havn på Samsø. Frisk fisk, hjemmelavede røgvarer, fiskefrikadeller og sæsonens udvalg siden 2009.'
    },
    nav: {
      home: 'Hjem',
      products: 'Produkter',
      history: 'Historie',
      contact: 'Kontakt',
      primary: 'Hovednavigation',
      skip: 'Spring til hovedindhold',
      openMenu: 'Åbn menu',
      closeMenu: 'Luk menu'
    },
    hero: {
      title: 'Ballen Fisk',
      subtitle: 'Kvalitetsfisk & Hjemmelavede Røgvarer',
      since: '<strong>Ballen Fisk siden 2009</strong> • <strong>Ballen Havn</strong>',
      sinceMobile: 'Ballen Fisk • Siden 2009',
      description: 'Velkommen til <strong>Samsøs</strong> foretrukne fiskehandler ved smukke <strong>Ballen Havn</strong>. Vi tilbyder dagligt <strong>friskfanget fisk</strong> fra Kattegat, hjemmerøgede specialiteter fra vores eget <strong>røgeri</strong> og de bedste <strong>fiskefrikadeller</strong> på øen – alt lavet med håndværk og passion.',
      cta: 'Se Vores Udvalg',
      smokehouseTitle: 'Eget Røgeri',
      smokehouseSubtitle: 'Røget på gammeldags manér'
    },
    status: {
      open: 'Åben',
      closed: 'Lukket',
      openingHours: 'Åbningstider',
      seasonShort: 'Sæsonåbningstider',

    },
    history: {
      title: 'Vores Historie',
      founded: 'Fiskeri fra kutteren',
      oldVehicle: 'Den gamle fiskevogn købes',
      founder: 'Drevet af Mogens Hede Vollbrecht',
      story1: 'I 1997 begyndte Mogens Hede Vollbrecht – bedre kendt som Fiskermogens – at fiske fra kutteren ved Samsøs kyster.',
      story2: 'På grund af stigende efterspørgsel på fisk og færre fisk i havet købte han i 2009 den gamle fiskevogn, hvorfra Ballen Fisk kunne betjene både lokale og sommergæster.',
      story3: 'I 2015 blev terrassen lavet, så gæster kunne nyde frisk fisk med udsigt over Ballen Havn.',
      milestoneTitle: 'En ny fiskevogn',
      milestoneDesc: 'I 2019 blev den nye fiskevogn indviet. Den markerede næste kapitel for Ballen Fisk og er i dag rammen om butikkens udvalg af frisk fisk, røgvarer og hjemmelavede specialiteter.',
      terrace: 'Terrassen bliver lavet',
      newVehicle: 'Ny fiskevogn indviet'
    },
    products: {
      title: 'Vores Udvalg',
      categories: 'Kvalitetsvarer fra Kattegat',
      freshFish: 'Fersk Fisk',
      smokedFish: 'Røget Fisk',
      fishCakes: 'Fiskefrikadeller & Fileter',
      readyMeals: 'Færdigretter',
      salads: 'Salater & Delikatesser',
      frozen: 'Frostvarer',
      beverages: 'Drikkevarer',
      fritureRejer: 'Friture rejer',
      misc: 'Diverse',
      disclaimer: 'Udvalg, priser og varebeholdning varierer efter sæson og daglig fangst. Spørg gerne i butikken for dagens udvalg.',
      noPhoto: 'Foto ikke tilgængeligt',
      prev: 'Forrige',
      next: 'Næste',
      askStore: 'Spørg i butikken',
      item: 'vare',
      items: 'varer',
      all: 'Alle'
    },
    gallery: {
      title: 'Et kig fra Ballen Fisk',
      subtitle: 'Frisk fisk, havneliv og terrassen ved smukke Ballen Havn.',
      label: 'Billedegalleri',
      previous: 'Forrige billede',
      next: 'Næste billede',
      chooseSlide: 'Vælg billede',
      goTo: 'Gå til billede',
      images: {
        oldHero: 'Den gamle fiskevogn ved Ballen Havn',
        shopFront: 'Ballen Fisk ved havnen',
        shopCloseUp: 'Et nærmere kig på butikken',
        fishCounter: 'Fisk i butiksdisken',
        fishOnIce: 'Frisk fisk på is',
        terraceHarbor: 'Terrassen med udsigt over havnen',
        terraceExterior: 'Terrassen udefra',
        fishTerrace: 'Fisketerrassen ved havnen',
        shopFrontSky: 'Menu ved Ballen Fisk',
        lobsterHandling: 'Hummer fra dagens udvalg',
        liveLobster: 'Hummere klar til butikken'
      }
    },
    contact: {
      title: 'Besøg Os',
      intro: 'Find os ved smukke <strong>Ballen Havn</strong> på <strong>Samsø</strong> – nyd <strong>frisk fisk</strong> på vores terrasse med udsigt over havnen, eller tag vores hjemmelavede <strong>røgvarer</strong> og <strong>fiskefrikadeller</strong> med hjem.',
      address: 'Adresse',
      street: 'Havnevej 3',
      postalCity: '8305 Samsø',
      country: 'Danmark',
      phone: 'Telefon',
      directions: 'Få rutevejledning',
      email: 'Email',
      facebook: 'Følg Os På Facebook',
      mapTitle: 'Ballen Fisk på kortet',
      season: 'Sæsonåbningstider',
      schedule: [
        { period: 'Påskeferie – 1. maj', hours: 'Søndag–torsdag 10:00–16:00 · Fredag–lørdag 10:00–18:00' },
        { period: '2. maj – 25. juni', hours: 'Alle dage 10:00–18:00' },
        { period: '26. juni – 7. august', hours: 'Søndag–torsdag 10:00–20:00 · Fredag–lørdag 10:00–21:00' },
        { period: '8. august – 31. august', hours: 'Alle dage 10:00–20:00' },
        { period: '1. september – 30. september', hours: 'Alle dage 10:00–18:00' },
        { period: '1. oktober – efterårsferie', hours: 'Søndag–torsdag 10:00–16:00 · Fredag–lørdag 10:00–18:00' }
      ]
    },
    footer: {
      copyright: '© {year} Ballen Fisk. Alle rettigheder forbeholdes.',
      since: 'Fiskeri fra Samsø siden 1997',
      tagline: 'Tradition & Kvalitet',
      madeWith: 'Lavet med',
      inLocation: 'på Samsø'
    }
  },
  en: {
    analytics: {
      title: 'Help us improve the site',
      description: 'We use anonymised usage data to understand what works on the site. Analytics is only activated if you accept.',
      accept: 'Accept analytics',
      decline: 'Decline'
    },
    seo: {
      title: 'Ballen Fisk | Fresh fish and smoked seafood on Samsø',
      description: 'Ballen Fisk at beautiful Ballen Harbor on Samsø. Fresh fish, homemade smoked seafood, fish cakes, and seasonal selection since 2009.'
    },
    nav: {
      home: 'Home',
      products: 'Products',
      history: 'History',
      contact: 'Contact',
      primary: 'Primary navigation',
      skip: 'Skip to main content',
      openMenu: 'Open menu',
      closeMenu: 'Close menu'
    },
    hero: {
      title: 'Ballen Fisk',
      subtitle: 'Quality Fish & Homemade Smoked Products',
      since: '<strong>Ballen Fisk since 2009</strong> • <strong>Ballen Harbor</strong>',
      sinceMobile: 'Ballen Fisk • Since 2009',
      description: 'Welcome to <strong>Samsø\'s</strong> favourite fishmonger at beautiful <strong>Ballen Harbor</strong>. We offer daily <strong>fresh-caught fish</strong> from the Kattegat, home-smoked specialities from our own <strong>smokehouse</strong>, and the island\'s best <strong>fish cakes</strong> – all crafted with tradition and passion.',
      cta: 'View Our Selection',
      smokehouseTitle: 'Own Smokehouse',
      smokehouseSubtitle: 'Smoked the Traditional Way'
    },
    status: {
      open: 'Open',
      closed: 'Closed',
      openingHours: 'Opening Hours',
      seasonShort: 'Seasonal hours',
      disclaimer: 'Always check our website for the most accurate times, as Google Maps may be outdated.'
    },
    history: {
      title: 'Our History',
      founded: 'Fishing from the Cutter',
      oldVehicle: 'The Old Fish Van Is Bought',
      founder: 'Run by Mogens Hede Vollbrecht',
      story1: 'In 1997, Mogens Hede Vollbrecht – known locally as Fishermogens – began fishing from the cutter along the shores of Samsø.',
      story2: 'As demand for fish grew and fewer fish remained in the sea, he bought the old fish van in 2009, serving locals and summer visitors from Ballen.',
      story3: 'In 2015, the terrace was built so guests could enjoy fresh fish overlooking Ballen Harbor.',
      milestoneTitle: 'A New Fish Van',
      milestoneDesc: 'The new fish van was inaugurated in 2019. It marked the next chapter for Ballen Fisk and is now home to the shop\'s selection of fresh fish, smoked goods, and homemade specialities.',
      terrace: 'The Terrace Is Built',
      newVehicle: 'New Fish Van Inaugurated'
    },
    products: {
      title: 'Our Selection',
      categories: 'Quality Goods from the Kattegat',
      freshFish: 'Fresh Fish',
      smokedFish: 'Smoked Fish',
      fishCakes: 'Fish Cakes & Fillets',
      readyMeals: 'Ready Meals',
      salads: 'Salads & Delis',
      frozen: 'Frozen Goods',
      beverages: 'Beverages',
      fritureRejer: 'Fried Shrimp',
      misc: 'Miscellaneous',
      disclaimer: 'Selection, prices, and availability vary by season and daily catch. Please ask in store about today’s selection.',
      noPhoto: 'Photo unavailable',
      prev: 'Previous',
      next: 'Next',
      askStore: 'Ask in store',
      item: 'item',
      items: 'items',
      all: 'All'
    },
    gallery: {
      title: 'A Look Around Ballen Fisk',
      subtitle: 'Fresh fish, harbour life, and the terrace at beautiful Ballen Harbor.',
      label: 'Photo gallery',
      previous: 'Previous image',
      next: 'Next image',
      chooseSlide: 'Choose image',
      goTo: 'Go to image',
      images: {
        oldHero: 'The old fish van at Ballen Harbor',
        shopFront: 'Ballen Fisk at the harbour',
        shopCloseUp: 'A closer look at the shop',
        fishCounter: 'Fish in the shop counter',
        fishOnIce: 'Fresh fish on ice',
        terraceHarbor: 'The terrace overlooking the harbour',
        terraceExterior: 'The terrace from outside',
        fishTerrace: 'The fish terrace by the harbour',
        shopFrontSky: 'The menu at Ballen Fisk',
        lobsterHandling: 'Lobster from today’s selection',
        liveLobster: 'Lobsters ready for the shop'
      }
    },
    contact: {
      title: 'Visit Us',
      intro: 'Find us at beautiful <strong>Ballen Harbor</strong> on <strong>Samsø</strong> – enjoy <strong>fresh fish</strong> on our terrace overlooking the harbour, or take our homemade <strong>smoked goods</strong> and <strong>fish cakes</strong> home with you.',
      address: 'Address',
      street: 'Havnevej 3',
      postalCity: '8305 Samsø',
      country: 'Denmark',
      phone: 'Phone',
      directions: 'Get directions',
      email: 'Email',
      facebook: 'Follow Us On Facebook',
      mapTitle: 'Ballen Fisk on the map',
      season: 'Seasonal opening hours',
      schedule: [
        { period: 'Easter break – 1 May', hours: 'Sunday–Thursday 10:00–16:00 · Friday–Saturday 10:00–18:00' },
        { period: '2 May – 25 June', hours: 'Daily 10:00–18:00' },
        { period: '26 June – 7 August', hours: 'Sunday–Thursday 10:00–20:00 · Friday–Saturday 10:00–21:00' },
        { period: '8 August – 31 August', hours: 'Daily 10:00–20:00' },
        { period: '1 September – 30 September', hours: 'Daily 10:00–18:00' },
        { period: '1 October – autumn break', hours: 'Sunday–Thursday 10:00–16:00 · Friday–Saturday 10:00–18:00' }
      ]
    },
    footer: {
      copyright: '© {year} Ballen Fisk. All rights reserved.',
      since: 'Fishing from Samsø since 1997',
      tagline: 'Tradition & Quality',
      madeWith: 'Made with',
      inLocation: 'in Samsø'
    }
  },
  de: {
    analytics: {
      title: 'Helfen Sie uns, die Seite zu verbessern',
      description: 'Wir verwenden anonymisierte Nutzungsdaten, um die Website zu verbessern. Statistiken werden nur aktiviert, wenn Sie zustimmen.',
      accept: 'Statistik akzeptieren',
      decline: 'Ablehnen'
    },
    seo: {
      title: 'Ballen Fisk | Frischer Fisch und Räucherwaren auf Samsø',
      description: 'Ballen Fisk am schönen Ballen Hafen auf Samsø. Frischer Fisch, hausgemachte Räucherwaren, Fischfrikadellen und saisonale Auswahl seit 2009.'
    },
    nav: {
      home: 'Startseite',
      products: 'Produkte',
      history: 'Geschichte',
      contact: 'Kontakt',
      primary: 'Hauptnavigation',
      skip: 'Zum Hauptinhalt springen',
      openMenu: 'Menü öffnen',
      closeMenu: 'Menü schließen'
    },
    hero: {
      title: 'Ballen Fisk',
      subtitle: 'Qualitätsfisch & Hausgemachte Räucherprodukte',
      since: '<strong>Ballen Fisk seit 2009</strong> • <strong>Ballen Hafen</strong>',
      sinceMobile: 'Ballen Fisk • Seit 2009',
      description: 'Willkommen bei <strong>Samsøs</strong> beliebtestem Fischhändler am schönen <strong>Ballen Hafen</strong>. Wir bieten täglich <strong>frisch gefangenen Fisch</strong> aus dem Kattegat, hausgeräucherte Spezialitäten aus unserer eigenen <strong>Räucherei</strong> und die besten <strong>Fischfrikadellen</strong> der Insel – alles mit Handwerk und Leidenschaft zubereitet.',
      cta: 'Produkte Ansehen',
      smokehouseTitle: 'Eigene Räucherei',
      smokehouseSubtitle: 'Traditionell geräuchert'
    },
    status: {
      open: 'Offen',
      closed: 'Geschlossen',
      openingHours: 'Öffnungszeiten',
      seasonShort: 'Saisonale Öffnungszeiten',
      disclaimer: 'Bitte prüfen Sie immer unsere Website für aktuelle Zeiten, da Google Maps veraltet sein kann.'
    },
    history: {
      title: 'Unsere Geschichte',
      founded: 'Fischerei vom Kutter',
      oldVehicle: 'Der alte Fischwagen wird gekauft',
      founder: 'Geführt von Mogens Hede Vollbrecht',
      story1: '1997 begann Mogens Hede Vollbrecht – lokal bekannt als Fischermogens – mit dem Kutter an den Küsten von Samsø zu fischen.',
      story2: 'Aufgrund der steigenden Nachfrage nach Fisch und weniger Fischen im Meer kaufte er 2009 den alten Fischwagen, um Einheimische und Sommergäste in Ballen zu bedienen.',
      story3: '2015 wurde die Terrasse gebaut, damit Gäste frischen Fisch mit Blick auf den Ballen Hafen genießen konnten.',
      milestoneTitle: 'Ein neuer Fischwagen',
      milestoneDesc: '2019 wurde der neue Fischwagen eingeweiht. Er markierte das nächste Kapitel für Ballen Fisk und beherbergt heute das Sortiment an frischem Fisch, Räucherwaren und hausgemachten Spezialitäten.',
      terrace: 'Die Terrasse wird gebaut',
      newVehicle: 'Neuer Fischwagen eingeweiht'
    },
    products: {
      title: 'Unser Sortiment',
      categories: 'Qualitätswaren aus dem Kattegat',
      freshFish: 'Frischer Fisch',
      smokedFish: 'Geräucherter Fisch',
      fishCakes: 'Fischfrikadellen & Filets',
      readyMeals: 'Fertiggerichte',
      salads: 'Salate & Delikatessen',
      frozen: 'Tiefkühlprodukte',
      beverages: 'Getränke',
      fritureRejer: 'Fritierte Garnelen',
      misc: 'Verschiedenes',
      disclaimer: 'Auswahl, Preise und Verfügbarkeit variieren je nach Saison und Tagesfang. Fragen Sie gerne im Laden nach dem aktuellen Sortiment.',
      noPhoto: 'Kein Foto verfügbar',
      prev: 'Zurück',
      next: 'Weiter',
      askStore: 'Im Laden fragen',
      item: 'Artikel',
      items: 'Artikel',
      all: 'Alle'
    },
    gallery: {
      title: 'Ein Blick auf Ballen Fisk',
      subtitle: 'Frischer Fisch, Hafenleben und die Terrasse am schönen Ballen Hafen.',
      label: 'Bildergalerie',
      previous: 'Vorheriges Bild',
      next: 'Nächstes Bild',
      chooseSlide: 'Bild auswählen',
      goTo: 'Zu Bild',
      images: {
        oldHero: 'Der alte Fischwagen am Ballen Hafen',
        shopFront: 'Ballen Fisk am Hafen',
        shopCloseUp: 'Ein näherer Blick auf den Laden',
        fishCounter: 'Fisch in der Ladentheke',
        fishOnIce: 'Frischer Fisch auf Eis',
        terraceHarbor: 'Die Terrasse mit Blick auf den Hafen',
        terraceExterior: 'Die Terrasse von außen',
        fishTerrace: 'Die Fischterrasse am Hafen',
        shopFrontSky: 'Die Speisekarte von Ballen Fisk',
        lobsterHandling: 'Hummer aus dem heutigen Sortiment',
        liveLobster: 'Hummer für den Laden'
      }
    },
    contact: {
      title: 'Besuchen Sie Uns',
      intro: 'Finden Sie uns am schönen <strong>Ballen Hafen</strong> auf <strong>Samsø</strong> – genießen Sie <strong>frischen Fisch</strong> auf unserer Terrasse mit Hafenblick, oder nehmen Sie unsere hausgeräucherten <strong>Spezialitäten</strong> und <strong>Fischfrikadellen</strong> mit nach Hause.',
      address: 'Adresse',
      street: 'Havnevej 3',
      postalCity: '8305 Samsø',
      country: 'Dänemark',
      phone: 'Telefon',
      directions: 'Route planen',
      email: 'E-Mail',
      facebook: 'Folgen Sie Uns Auf Facebook',
      mapTitle: 'Ballen Fisk auf der Karte',
      season: 'Saisonale Öffnungszeiten',
      schedule: [
        { period: 'Osterferien – 1. Mai', hours: 'Sonntag–Donnerstag 10:00–16:00 · Freitag–Samstag 10:00–18:00' },
        { period: '2. Mai – 25. Juni', hours: 'Täglich 10:00–18:00' },
        { period: '26. Juni – 7. August', hours: 'Sonntag–Donnerstag 10:00–20:00 · Freitag–Samstag 10:00–21:00' },
        { period: '8. August – 31. August', hours: 'Täglich 10:00–20:00' },
        { period: '1. September – 30. September', hours: 'Täglich 10:00–18:00' },
        { period: '1. Oktober – Herbstferien', hours: 'Sonntag–Donnerstag 10:00–16:00 · Freitag–Samstag 10:00–18:00' }
      ]
    },
    footer: {
      copyright: '© {year} Ballen Fisk. Alle Rechte vorbehalten.',
      since: 'Fischerei von Samsø seit 1997',
      tagline: 'Tradition & Qualität',
      madeWith: 'Mit Liebe gemacht',
      inLocation: 'auf Samsø'
    }
  }
};
